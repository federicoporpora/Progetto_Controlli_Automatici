...
\end{align}
%
Calcoliamo preliminarmente la matrice $(sI-A)$ e il suo determinante (polinomio caratteristico):
%
\begin{equation}
    (sI-A) = 
    \begin{bmatrix}
        s + 0.3262 & 0 \\
        -4.8925 & s + 0.0313
    \end{bmatrix}, \quad \Delta_A(s) = (s + 0.3262)(s + 0.0313).
\end{equation}
%
Svolgendo i calcoli matriciali, si ottiene:
%
\begin{align}
    G(s) &= \begin{bmatrix} 0 & 1 \end{bmatrix} \frac{1}{\Delta_A(s)} 
    \begin{bmatrix}
        s + 0.0313 & 0 \\
        4.8925 & s + 0.3262
    \end{bmatrix}
    \begin{bmatrix} 2.5000 \\ 0 \end{bmatrix} \nonumber \\
    &= \frac{1}{\Delta_A(s)} \begin{bmatrix} 4.8925 & s + 0.3262 \end{bmatrix} \begin{bmatrix} 2.5000 \\ 0 \end{bmatrix} \nonumber \\
    &= \frac{12.2313}{(s + 0.3262)(s + 0.0313)}.
\end{align}
%
Dunque il sistema linearizzato~\eqref{eq:linearized_system} è caratterizzato dalla funzione di trasferimento~\eqref{eq:transfer_function} con 2 poli reali negativi $p_1 = -0.3262$, $p_2 = -0.0313$ e nessun zero. In Figura~\ref{fig:bode_diagram} mostriamo il corrispondente diagramma di Bode.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{figs/bode.jpg}
    \caption{Diagramma di Bode della funzione di trasferimento $G(s)$.}
    \label{fig:bode_diagram}
\end{figure}

Inoltre, analizzando la funzione di trasferimento, possiamo fare alcune considerazioni preliminari sul comportamento del sistema:
\begin{itemize}
    ...
    \item \textbf{Assenza di oscillazioni:} Essendo i poli reali (e non complessi coniugati), la risposta al gradino del sistema a catena aperta sarà di tipo aperiodico (sovrasmorzato), ovvero non presenterà sovraelongazioni naturali.
    \item \textbf{Guadagno statico:} Il guadagno statico è $\mu = G(0) \approx 1198$ (circa $61.6$ dB). Questo valore elevato indica che una piccola variazione di tensione della pompa provoca, a regime, una grande variazione di livello nel secondo serbatoio.
\end{itemize}
