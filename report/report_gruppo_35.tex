...
\end{subequations}
%
dove $a_1(t)$ e $a_2(t)$ rappresentano i livelli dei serbatoi, $V(t)$ la tensione della pompa e $k_i$ 
sono parametri geometrici.
...
%
\begin{align*}
    \delta x &= x - x_e, \quad \delta u = u 
- u_e, \quad \delta y = y - y_e.
\end{align*}
%
...
\begin{itemize}
    \item[1)] \textbf{Errore a regime:} $|e_{\infty}|
\le 0.05$ per riferimento $W=3$ e disturbo $D=2.5$.
...
%
Il tempo di assestamento al 5\% impone un vincolo sulla velocità di risposta e quindi sulla pulsazione critica $\omega_c$.
...
%
La sovraelongazione massima $S\% \le 20\%$ richiede un coefficiente di smorzamento adeguato.
Si assume come target progettuale un margine di fase $M_f \ge 55^\circ$, più stringente del requisito minimo di $30^\circ$.
...
%
Il progetto del regolatore inizia con la determinazione del guadagno statico $\mu_s$ necessario per soddisfare i requisiti di precisione a regime.
%
...
%
\begin{equation}
    e_{\infty} = \frac{W - D \mu_G}{1 + \mu_s \mu_G}
\end{equation}
%
...
%
\begin{equation}
    \mathbf{\mu_s = 250 \quad (47.96 \text{ dB})}
\end{equation}

Dunque, definiamo la funzione estesa $G_e(s) = R_s(s)G(s) = \mu_s G(s)$ e, in Figura \ref{fig:bode_diagram_Ge}, mostriamo il suo diagramma di Bode 
per verificare se e quali zone proibite vengono attraversate.
...
%
Il regolatore finale $R(s) = \mu_s R_d(s)$ risulta quindi:
%
\begin{equation}
...
In particolare:
\begin{itemize}
    \item \textbf{Reiezione disturbi:} Il guadagno elevato assicura $|L|
\ge 40$ dB per $\omega \le 2$ rad/s.
    \item \textbf{Attenuazione rumore:} La pendenza del sistema e del controllore garantiscono $|L|
\le -63$ dB per $\omega \ge 10^5$ rad/s.
...
Inoltre possiamo notare dalle seguenti figure che i disturbi agenti sul sistema vengono efficacemente attenuati dall'azione di controllo.
La Figura~\ref{fig:rej_dist} mostra la reiezione del disturbo $d(t)$ a bassa frequenza, mentre la Figura~\ref{fig:rej_noise} evidenzia l'abbattimento del rumore di misura $n(t)$.
\begin{figure}[H]
    \centering
...
\subsection{Analisi dello sforzo di controllo}

In aggiunta all'analisi dell'uscita, per completezza si analizza lo 
sforzo di controllo $\delta u$ richiesto per ottenere le prestazioni teoriche lineari.
\begin{figure}[H]
    \centering
...
%
Questo suggerisce che, in un'implementazione fisica reale, l'attuatore (pompa) andrebbe in saturazione molto prima di raggiungere tali valori.
La saturazione limiterebbe l'energia fornita al sistema, rallentando ulteriormente la risposta rispetto al caso ideale.
Per un'applicazione pratica, sarebbe necessario rilassare la specifica sul tempo di assestamento.
...
%
Rispetto alle simulazioni riguardanti il sistema linearizzato emerge una sovraelongazione maggiore dovuta alle non linearità.
Tuttavia, il sistema riesce a inseguire il riferimento e a mantenere la stabilità nonostante la presenza di forti disturbi esterni.
...
%
L'obiettivo è verificare se, partendo da stati perturbati (serbatoi vuoti o troppo pieni), il controllore riesce a riportare il sistema all'equilibrio $x_e$ senza incorrere in instabilità o violazioni fisiche.

Come mostrato nella griglia di figure sottostante (Figura~\ref{fig:grid_initial_conditions}), il sistema dimostra una notevole robustezza. Per tutte le condizioni iniziali testate, anche quelle estreme, l'uscita del sistema converge asintoticamente all'equilibrio.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_1.jpg}
        \caption{Test 1: $a_1(0)=0.005, a_2(0)=3.67$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_2.jpg}
        \caption{Test 2: $a_1(0)=0.10, a_2(0)=3.67$.}
    \end{subfigure}
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_3.jpg}
        \caption{Test 3: $a_1(0)=0.0235, a_2(0)=1.0$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_4.jpg}
        \caption{Test 4: $a_1(0)=0.0235, a_2(0)=5.0$.}
    \end{subfigure}
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_5.jpg}
        \caption{Test 5: $a_1(0)=0.01, a_2(0)=2.0$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_6.jpg}
        \caption{Test 6: $a_1(0)=0.05, a_2(0)=5.0$.}
    \end{subfigure}
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_7.jpg}
        \caption{Test 7: $a_1(0)=0.005, a_2(0)=6.0$.}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figs/cond_ini_8.jpg}
        \caption{Test 8: $a_1(0)=0.10, a_2(0)=1.0$.}
    \end{subfigure}
    
    \caption{Risposta del sistema non lineare per diverse condizioni iniziali $a_1(0)$ e $a_2(0)$. In tutti i casi il sistema converge all'origine dello spazio delle variazioni (condizione di equilibrio).}
    \label{fig:grid_initial_conditions}
\end{figure}

Questo suggerisce che il bacino di attrazione dell'equilibrio, per il sistema controllato, copre praticamente l'intero spazio operativo fisico dei serbatoi.
...
%
Incrementando progressivamente l'ampiezza del gradino, non è stato osservato un punto di rottura o instabilità esplicita nelle simulazioni effettuate. Il sistema tende sempre a convergere al valore di riferimento desiderato, anche per ampiezze elevate che portano alla saturazione fisica degli attuatori o allo svuotamento transitorio dei serbatoi.
%
In questi casi limite, il tempo di assestamento aumenta considerevolmente a causa dei vincoli fisici (la pompa non può erogare tensione infinita e il flusso è limitato dalla geometria), ma l'azione integrale e la stabilità intrinseca del processo idraulico garantiscono che l'uscita raggiunga infine il set-point senza divergere.
...
%
L'analisi approfondita sul modello non lineare ha confermato la validità del progetto: sebbene le non linearità introducano sovraelongazioni maggiori e rallentino la risposta in caso di ampie variazioni del riferimento (saturazione), il sistema ha dimostrato un'eccellente robustezza, convergendo all'equilibrio per un ampissimo range di condizioni operative e iniziali.
\end{document}